...
module SageNetwork
  class ApiRecord
    include SageNetwork::ApiRecords::Scopes
    # SageNetwork::ApiRecord provides an easy way to use Ruby to interace with a api.lockstep.io backend
    # Usage:
    #  class Post < SageNetwork::ApiRecord
    ...
    # Instantiates a SageNetwork::ApiRecord object
    #
    # @params [Hash], [Boolean] a `Hash` of attributes and a `Boolean` that should be false only if the object already exists
    # @return [SageNetwork::ApiRecord] an object that subclasses `Parseresource::Base`
    def initialize(attributes = {}, new = true)
      ...
      attributes.each do |k, v|
        ...
        if (type = schema[k])
          attributes[k] = type[v]
        elsif v.present? && (enum = enum_config[k]).present? && enum.keys.include?(v.to_s)
          attributes[k] = enum[v]
        end
      end
    ...
    # @polymorphic is used to assign polymorphic association with the related ApiModel. It expects a hash with the key-value, or a lambda function
    ...
    def create_getters!(key, _val)
      return if respond_to? key.to_s

      self.class.send(:define_method, key.to_s) do
        get_attribute(key.to_s)
      end
    end
    ...
    def self.config
      @config ||= Rails.application.config_for(:sage_network_client)
    end

    # Gets the current class's SageNetwork.io base_uri
    def self.model_base_uri
      if name.starts_with?('Schema::')
        raise StandardError,
              'Cannot establish connection for auto-generated Schema. Create a new model if you want to retrieve data from SageNetwork Platform'
      end
    ...
    def self.resource
      ...
      SageNetwork::Client.new(model_base_uri)
    end
    ...
    def self.merge_all_attributes(objects, response)
      objects.each_with_index do |item, index|
        ...
        new_attributes = response[index].transform_keys(&:underscore)
    ...
    def self.bulk_import(new_objects, slice_size = nil)
      ...
      new_objects.each_slice(slice_size) do |objects|
        ...
        if resp.code != '200'
          ...
          #   }
          if resp.code == '401'
            raise SageNetwork::Exceptions::UnauthorizedError, 'Unauthorized: Check your App ID & Master Key'
          elsif resp.code == '400'
            raise SageNetwork::Exceptions::BadRequestError, JSON.parse(resp.body)
          elsif resp.code == '404'
            raise SageNetwork::Exceptions::RecordNotFound, 'Resource not found in the Platfrom'
          end
        end
        ...
        #   SageNetwork::Contact.new(item.transform_keys { |key| key.underscore }, false)
        # }
    ...
    # Find a SageNetwork::ApiRecord object by ID
    #
    # @param [String] id the ID of the Parse object you want to find.
    # @return [SageNetwork::ApiRecord] an object that subclasses SageNetwork::ApiRecord.
    def self.find(id)
      raise SageNetwork::Exceptions::RecordNotFound, "Couldn't find #{name} without an ID" if id.blank?

      record = where(id_ref => id).first
      raise SageNetwork::Exceptions::RecordNotFound, "Couldn't find #{name} with id: #{id}" if record.blank?

      record
    end

    # Find a SageNetwork::ApiRecord object by given key/value pair
    #
    def self.find_by(*args)
      raise SageNetwork::Exceptions::RecordNotFound, "Couldn't find an object without arguments" if args.blank?

      key, value = args.first.first
      raise StandardError, "Attribute '#{key}' has not been defined for #{name}" unless valid_attribute?(key, raise_exception: true)

      where(key => value).first
    end

    # Find a SageNetwork::ApiRecord object by chaining #where method calls.
    #
    def self.where(*args)
    ...
    include SageNetwork::QueryMethods

    def self.chunk(attribute)
      query_builder.chunk(attribute)
    end

    # Create a SageNetwork::ApiRecord object.
    #
    # @param [Hash] attributes a `Hash` of attributes
    # @return [SageNetwork::ApiRecord] an object that subclasses `SageNetwork::ApiRecord`. Or returns `false` if object fails to save.
    ...
    def self.valid_attribute?(key, raise_exception: false)
      ...
      return true if name == 'SageNetwork::ApiRecord'

      attr = key.to_s
      SageNetwork::Query::PREDICATES.keys.each do |predicate|
    ...
    def post_result(resp)
      if resp.code.to_s == '200' || resp.code.to_s == '201'
        ...
        pe = if error_response['error']
               SageNetwork::Error.new(error_response['code'], error_response['error'])
             else
               SageNetwork::Error.new(resp.code.to_s)
             end
    ...
    def get_attribute(k)
      ...
      when Hash
        ...
        when 'Relation'
          ...
          attrs[k] = SageNetwork::RelationArray.new self, objects_related_to_self, k, klass_name
          @unsaved_attributes[k] = SageNetwork::RelationArray.new self, objects_related_to_self, k, klass_name
    ...
    # if we are comparing objects, use id if they are both SageNetwork::ApiRecord objects
    def ==(other)
      if other.class <= SageNetwork::ApiRecord
        id == other.id
      else
        super
      end
      ...
      when :has_many
        ...
        if relation_config[:loader].present?
          ...
          if relation_config[:polymorphic]
            polymorphic_config = SageNetwork::RelationArray.has_many_polymorphic_attributes(self,
                                                                                         relation_config[:polymorphic])
            query.merge!(polymorphic_config)
          end
          related_objects = relation_klass.send(:where, query).execute
          val = SageNetwork::RelationArray.new self, related_objects, relation, relation_config[:class_name]
        end
      when :belongs_to
        relation_config = belongs_to_relations[relation]
        val = if relation_config[:loader].present?
                relation_config[:loader].call(self)
              else
                relation_config[:class_name].constantize.send(:find_by,
    ...
    def self.single_record!
      define_singleton_method :record do
        ...
        raise StandardError, "#{resp.code} error while fetching: #{resp.body}" unless %w[201 200].include?(resp.code.to_s)
