class SageNetwork::Query
  ...
  #   unscoped = SageNetwork::Connection.unscoped
  ...
  def convert_arg(arg)
    return arg.to_pointer if arg.is_a?(SageNetwork::ApiRecord)
    return SageNetwork::ApiRecord.to_date_object(arg) if arg.is_a?(Time) || arg.is_a?(Date)
  ...
  def build_params
    ...
    # SageNetwork Platform API does not support a page size of 1
      ...
      objects.each do |item|
        @klass.has_many_relations.each do |relation, relation_config|
          ...
          if !item.attributes.has_key?(relation.to_s)
            item.attributes[relation.to_s] = SageNetwork::RelationArray.new(@klass, [], relation, relation_config[:class_name])
          elsif !item.attributes[relation].is_a?(SageNetwork::RelationArray)
            item.attributes[relation.to_s] = SageNetwork::RelationArray.new(@klass, item.attributes[relation], relation, relation_config[:class_name])
          end
        end
      end
    end
