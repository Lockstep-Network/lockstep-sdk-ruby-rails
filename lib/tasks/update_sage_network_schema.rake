...
task :update_sage_network_schema do
  # download_schema!
  create_models
end
...
SCHEMA_SWAGGER_URL = 'https://lockstep-app-store-api-dev-v2.azurewebsites.net/swagger/v2/swagger.json'
SCHEMA_FILE_PATH = 'app/sage_network/swagger.json'
TEMPLATE = 'app/sage_network/model_template.rb.erb'
SCHEMA_PATH = 'app/sage_network/schema/sage_network'
...
def create_models
  JSON.parse(File.read(SCHEMA_FILE_PATH))['components']['schemas'].each do |schema_name, schema|
    next unless schema['type'] == 'object'
    next unless schema['properties'].instance_of?(Hash)
...
def class_name(name)
  len = name.length
  "SageNetwork::#{name[0].upcase}#{name[1..len - 1]}".gsub('Model', '')
end

def build_fields(_schema_name, properties)
  fields = {}
  properties.each do |property_name, property|
    next if !property['items'].nil? && !property['items']['$ref'].nil?
...
BELONGS_TO_RELATIONS = {
  connection: {
    keys: %w[customerId],
    class_name: 'SageNetwork::Connection',
    primary_key: :company_id
  },
  account: {
    keys: %w[companyId],
    class_name: 'SageNetwork::Account',
    primary_key: :company_id
  },
  created_user: {
    keys: %w[createdUserId],
    class_name: 'SageNetwork::User',
    primary_key: :user_id
  },
  modified_user: {
...
def build_belongs_to_relations(_schema_name, properties)
  belongs_to_relations = {}
  properties.each do |property_name, _property|
    BELONGS_TO_RELATIONS.each do |relation_name, config|
      next unless config[:keys].include?(property_name)

      belongs_to_relations[underscore(property_name).to_s.gsub('_id', '')] =
        { class_name: config[:class_name], primary_key: config[:primary_key], foreign_key: underscore(property_name) }
      belongs_to_relations[underscore(relation_name)] =
        { class_name: config[:class_name], primary_key: config[:primary_key], foreign_key: underscore(property_name) }
    end
  end
  belongs_to_relations
...
def build_has_many_relations(schema_name, properties)
  ...
  properties.each do |property_name, property|
    ...
    case property_name
    when 'notes'
      has_many_relations[underscore(property_name)] =
        { class_name: 'SageNetwork::Note', included: true, foreign_key: :object_key, polymorphic: { table_key: schema_name.gsub('Model', '') } }
    else
      has_many_relations[underscore(property_name)] = { class_name: "Schema::SageNetwork::#{ref}", included: true }
    end
  end
  has_many_relations
end
